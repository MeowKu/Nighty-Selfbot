        ...
        f.write("""
{
    ...
    if not os.path.isfile('giveawaybots.json'):
        ...
        f.write("""
{
    "294882584201003009": "ðﾟﾎﾉ",
    "396464677032427530": "ðﾟﾎﾉ",
    "720351927581278219": "ðﾟﾎﾉ",
    "582537632991543307": "ðﾟﾎﾉ"
}
        """)
        f.close()        
...
async def example(Ghost):
    ...
    **\_\_deletetimeout\_\_** : Config delete timeout (seconds).
    ...
    cycleStatusText = ""
    cycleStatus = False
    discordServer = "https://dsc.gg/sansserver"
    uwuifyEnabled = False
    channelBlankChar = "᲼"
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                          
        ...
        if duration is not None:
            if winners is not None:
                if prize is not None:
                    ...
                    e = discord.Embed(
                        description=f"React with ðﾟﾎﾉ to enter!\nEnds in {timepretty}\nHosted by {ctx.author.mention}",
                        color=__embedcolour__)
                    if winners >= 2:
                    ...
                    e.timestamp = future
                    msg = await ctx.send("ðﾟﾎﾉ **GIVEAWAY** ðﾟﾎﾉ", embed=e)
                    ...
                    if "ðﾟﾎﾉ **GIVEAWAY** ðﾟﾎﾉ" in msg.content:
                        entries = []
                        ...
                        if nowinner is True:
                            ...
                            await ctx.send("ðﾟﾎﾉ " + ', '.join(winnerslist) + f" you won **{prize}**\n{msg.jump_url}")
                            newe = discord.Embed(
                        ...
                        await msg.edit(content="ðﾟﾎﾉ **GIVEAWAY ENDED** ðﾟﾎﾉ", embed=newe)
    ...
    async def gend(ctx, id: int = None):
        ...
        if msgId == id and msgAuthorId == Ghost.user.id:
            if "ðﾟﾎﾉ **GIVEAWAY** ðﾟﾎﾉ" in msgContent:
                embeds = msg.embeds
                for embed in embeds:
            ...
            elif "ðﾟﾎﾉ **GIVEAWAY ENDED** ðﾟﾎﾉ" in msgContent:
                await ctx.send("ðﾟﾘﾔ That giveaway has already ended.")
    ...
    async def greroll(ctx, id: int = None):
        ...
        if msg.author.id == Ghost.user.id:
            ...
            elif "ðﾟﾎﾉ **GIVEAWAY ENDED** ðﾟﾎﾉ" in msg.content:
                embeds = msg.embeds
                for embed in embeds:
                    embed_dict = embed.to_dict()
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)
    ...
    async def selfbotcheck(ctx):
        ...
        await ctx.send("ðﾟﾎﾉ **GIVEAWAY** ðﾟﾎﾉ")
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)                              
    ...
    async def howgay(ctx, user: discord.User):
        ...
        if __embedmode__:
            embed = discord.Embed(title=f"ðﾟﾏﾳ️‍ðﾟﾌﾈ {user.name} is {percentage} gay", color=__embedcolour__)
            embed.set_thumbnail(url=__embedimage__)
            embed.set_footer(text=__embedfooter__, icon_url=__embedfooterimage__)
            embed.timestamp = datetime.now()
            await ctx.send(embed=embed)
        else:
            await ctx.send(f"ðﾟﾏﾳ️‍ðﾟﾌﾈ {user} is {percentage} gay")        
...
```""")
        ...
        emojis = [("ðﾟﾍﾒ", 0.01), ("ðﾟﾍﾊ", 0.02), ("ðﾟﾍﾎ", 0.06), ("ðﾟﾒﾎ", 0.08), ("ðﾟﾍﾆ", 0.14), ("ðﾟﾍﾉ", 0.24), ("ðﾟﾎﾰ", 0.36)]
        emojis2 = []
